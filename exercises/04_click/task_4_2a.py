...
В этом задании вместо передачи параметров подключения через yaml, используются
опции или запрос информации у пользователя.  Если параметры подключения
отличаются от указанных в примерах ниже, надо изменить их в файле devices.yaml
на свои, так как тест берет параметры из файла devices.yaml.
...
* опция --username, с коротким вариантом -u - имя пользователя. Если опция не
  указана при вызове скрипта, запросить ввод информации у пользователя
  (средствами click)
* опция --password, с коротким вариантом -p - пароль. Если опция не указана при
  вызове скрипта, запросить ввод информации у пользователя (средствами click).
  Скрывать ввод.
* опция --secret, с коротким вариантом -s - пароль на режим enable. Если опция
  не указана при вызове скрипта, запросить ввод информации у пользователя
  (средствами click). Скрывать ввод.
